...
contract SkullyBLONKSsvg {
    ...
    returns (string memory)
  {
    ...
    if (tA[0] == 0) {
      ...
      b = string(abi.encodePacked(b,'<defs><linearGradient gradientUnits="userSpaceOnUse" x1="500" y1="0" x2="500" y2="1000" id="bkStyle"><stop offset="0" style="stop-color: #000000"/><stop offset="1" style="stop-color: ',bC(tR, tG, tB),'</linearGradient></defs>'));
    } else if (tA[0] == 2) {
      b = string(abi.encodePacked(b,'<defs><radialGradient gradientUnits="userSpaceOnUse" cx="500" cy="500" r="700" id="bkStyle"><stop offset="0" style="stop-color: #000000"/><stop offset="1" style="stop-color: ',bC(tR, tG, tB),'</radialGradient></defs>'));
    } else {
      b = string(abi.encodePacked(b,'<defs><radialGradient gradientUnits="userSpaceOnUse" cx="500" cy="500" r="490" id="bkStyle"><stop offset=".5" style="stop-color: #000000"/><stop offset="1" style="stop-color: ',bC(tR, tG, tB),'</radialGradient></defs>'));
    }
    ...
    // Neck
    b = string(abi.encodePacked(b,rA(loc[0] + loc[1] / 3, loc[3] + loc[5], loc[1] / 3, 1010 - loc[3] - loc[5], 244, 241, 233, 10, 32)));

    // Head
    tR += 20;
    tG += 20;
    tB += 20;
    b = string(abi.encodePacked(b,rA(loc[2], loc[3], loc[4], loc[5], 249, 246, 238, 19, 42)));


    // Eyebrows
    b = string(abi.encodePacked(b,rA(loc[37], loc[38], loc[39], loc[40], 229, 226, 218, 0, 0)));
    b = string(abi.encodePacked(b,rA(loc[41], loc[42], loc[43], loc[44], 229, 226, 218, 0, 0)));

    // Nose
    b = string(abi.encodePacked(b,rA(loc[24], loc[25], loc[26], loc[27], 0, 0, 0, 6, 55)));
    ...
    // Eyes
    b = string(abi.encodePacked(b,rA(loc[6], loc[7], loc[8], loc[9], 0, 0, 0, 6, 55)));
    b = string(abi.encodePacked(b,rA(loc[10], loc[11], loc[12], loc[13], 0, 0, 0, 6, 55)));

    // Pupils
    if (tA[1] == 1) {
      s[0] = '(210,42,42); stroke-width: 6px; stroke: rgb(205,37,37);"/>';
    } else if (tA[1] == 2) {
      s[0] = '(70,219,44); stroke-width: 6px; stroke: rgb(65,214,39);"/>';
    } else {
      s[0] = '(255,255,255); stroke-width: 6px; stroke: rgb(55,55,55);"/>';
    }
    ...
    // jawline
    b = string(abi.encodePacked(b,rA(loc[2], loc[34], loc[4], 10, 42, 42, 42, 6, 42)));
    ...
    eT /= 10;
    b = string(abi.encodePacked(b,rA(loc[33], loc[34], loc[35], loc[36], 0, 0, 0, 6, 42)));

    // Teeth
    if (tA[2] > 0) {
      b = string(abi.encodePacked(b,rA(loc[50], loc[51], loc[52], loc[53], 249, 246, 238, 0, 0)));
    }
    if (tA[2] == 3) {
      b = string(abi.encodePacked(b,rA(loc[93], loc[94], loc[95], loc[96], 0, 0, 0, 0, 0))); // paints mouth over tusks
    }

    // Mole
    if (loc[45] > 0) {
      b = string(abi.encodePacked(b,rA(loc[45], loc[106], loc[107], loc[108], 229, 226, 218, 0, 0)));     
    } 

    // Extra Detail
    s[0] = '(222,22,22);"/>'; // blood red
    if (tA[3] > 0) {
    ...
    if (tA[4] != 0) {
      ...
      b = string(abi.encodePacked(b,rS(loc[2] - 20, loc[70], loc[71] - (loc[2] - loc[69]) + 20, loc[65], s[0])));
      b = string(abi.encodePacked(b,rS(loc[72], loc[73], (loc[2] + loc[4]) - loc[72] + 20, loc[65], s[0])));
    }
    ...
    if (tA[7] == 1 || tA[7] == 3 || tA[7] == 4) {
      b = string(abi.encodePacked(b,rS(loc[28] + (loc[31] - loc[48]) / 2, 1000 - loc[48], loc[48], loc[48], s[0])));
    }
    if (tA[7] == 2 || tA[7] == 3 || tA[7] == 4) {
      b = string(abi.encodePacked(b,rS(loc[29] + (loc[31] - loc[48]) / 2, 1000 - loc[48], loc[48], loc[48], s[0])));
    }
    s[0] = '(252,214,18);"/>';
    if (tA[8] == 1 || tA[8] == 3) { 
      b = string(abi.encodePacked(b,rS(loc[28] - 15, 985, 30, 15, s[0])));
    } 
    if (tA[8] == 2 || tA[8] == 3) { 
      b = string(abi.encodePacked(b,rS(loc[29] + loc[31] - 15, 985, 30, 15, s[0])));
    }
    ...
    // neck thickness adjustment  
    if (tA[9] != 1) {
      loc[98] = loc[98] + loc[100] / 3 - 5;
      loc[100] = loc[100] / 3 + 10;
    }
    ...
    if (tA[9] == 0) {
      ...
      b = string(abi.encodePacked(b,rS(loc[98] - 30, loc[99], loc[100] + 60, loc[101], '(252,214,18);"/>')));
      b = string(abi.encodePacked(b,rS(loc[98], loc[99], loc[100], loc[101], '(0,120,90);"/>')));
    }
