...
/// @title BLOOPS SVG Contract v1.3
/// @author Matto
...
contract BLOOPSsvg {
  ...
  function rS(uint16 _x, uint16 _y, uint16 _w, uint16 _h, string memory _style)
    ...
    returns (string memory)
  {
    return string(abi.encodePacked('<ellipse cx="',Strings.toString(_x),'" cy="',Strings.toString(_y),'" rx="',Strings.toString(_w),'" ry="',Strings.toString(_h),'" style="fill: rgb',_style));
  }
    ...
    returns (string memory)
  {
    ...
    // Neck
    b = string(abi.encodePacked(b,rA(loc[0], loc[3] + loc[5], loc[1], loc[5], tR, tG, tB, 10, 32)));
    ...
    s[0] = '(252,214,18);"/>';
    if (tA[8] == 1 || tA[8] == 3) { 
      b = string(abi.encodePacked(b,rS(loc[28] - 15, loc[49] - 80, 30, 15, s[0])));
    } 
    if (tA[8] == 2 || tA[8] == 3) { 
      b = string(abi.encodePacked(b,rS(loc[29] + loc[31] - 15, loc[49] - 80, 30, 15, s[0])));
    }
    ...
    tB = cS(tB, 40);
    if (tA[9] == 0) {
      b = string(abi.encodePacked(b,rA(loc[98],loc[99],loc[100],loc[101], tR, tG, tB, 6, 32)));
    } else if (tA[9] == 1) {
      ...
      b = string(abi.encodePacked(b,rS(loc[98],loc[99] + 20,loc[100],loc[101] + 20,'(8,8,8);"/>')));
    } else if (tA[9] == 5) {
      b = string(abi.encodePacked(b,rS(loc[98] + 20,loc[99] + 20,loc[100],loc[101],'(252,214,18);"/>')));
      b = string(abi.encodePacked(b,rS(loc[102] + 20,loc[99] + 20,loc[104],loc[101],'(0,120,90);"/>')));
    }
