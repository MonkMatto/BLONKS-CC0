...
/// @title GHOSTS SVG Contract v2.1 (fill-opacity version)
/// @author Matto
...
contract BLONKSsvg {
  ...
  function rA(uint16 _x, uint16 _y, uint16 _w, uint16 _h, uint16 _sw, uint16 _sc)
    internal
    pure
    returns (string memory)
  {
    return string(abi.encodePacked('<rect x="',Strings.toString(_x),'" y="',Strings.toString(_y),'" width="',Strings.toString(_w),'" height="',Strings.toString(_h),'" style="fill: rgb(0,0,0); fill-opacity: 0; stroke-width: ',Strings.toString(_sw),'px; stroke: rgb(',Strings.toString(_sc),', ',Strings.toString(_sc),', ',Strings.toString(_sc),');"/>'));
  }
